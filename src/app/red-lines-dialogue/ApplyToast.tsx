...
export default function RedLinesApplyToast() {
  const searchParams = useSearchParams();
  const router = useRouter();
  // "1": application is with the working group. "confirm": email-only path,
  // parked until the applicant uses the link we just emailed them.
  ...
  useEffect(() => {
    ...
    router.replace("/red-lines-dialogue#apply", { scroll: false });
