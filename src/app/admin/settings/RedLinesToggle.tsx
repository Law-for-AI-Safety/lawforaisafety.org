...
/**
 * The on/off button for the Red Lines Dialogues application form. Mirrors
 * SignupToggle.tsx.
 */
export default function RedLinesToggle({
  enabled,
  policyPublished,
}: {
  enabled: boolean;
  ...
  return (
    <form
      action="/api/admin/settings/red-lines"
      method="post"
    ...
    >
      ...
      {needsAcknowledgement && (
        <label className="flex items-start gap-3 border border-brand-red bg-brand-red/10 px-3 py-3 text-brand-black/80">
          ...
          />
          <span>
            The privacy policy is not published on this site. Turning
            applications on means collecting people&apos;s data with no
            notice they can read. Tick to turn it on anyway.
          </span>
        </label>
      )}
