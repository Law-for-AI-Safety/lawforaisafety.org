...
// Set when this page is running inside Decap's admin preview iframe
// (public/admin/preview.js), so a draft edit updates the page without a
// save. Anywhere else, nothing ever posts this message and `data` never
// moves off `initialData`.
function usePreviewData(initialData: HomeContent) {
  const [data, setData] = useState(initialData);

  useEffect(() => {
    if (window.self === window.top) return; // not embedded, skip listener

    function onMessage(event: MessageEvent) {
      if (event.origin !== window.location.origin) return;
      if (event.data?.type !== "decap-preview-update") return;
      setData(event.data.payload as HomeContent);
    }

    window.addEventListener("message", onMessage);
    // window.top, not window.parent: Decap renders the whole preview pane
    // inside its own iframe for style isolation, so this iframe (the real
    // site) is nested two levels deep. window.parent would only reach
    // Decap's internal preview iframe, which has no listener — window.top
    // always resolves to the actual outermost window (admin/index.html,
    // where public/admin/preview.js's listener lives), regardless of how
    // many intermediate frames Decap inserts.
    window.top?.postMessage({ type: "decap-preview-ready" }, window.location.origin);
...
export default function HomeClient({ initialData }: { initialData: HomeContent }) {
  const data = usePreviewData(initialData);
  const { hero, mission, work, quote, story, team, contact } = data;
