...
2. `/api/newsletter` validates the honeypot is empty, inserts a row into `newsletter_signups` (with a random `confirmation_token`, `confirmed_at = null`), and sends a **real double opt-in confirm-link email via Brevo's transactional API**, styled (see API Dependencies → Transactional Email). This is our own double opt-in mechanism, not Brevo's native `doubleOptinConfirmation` endpoint — see Implementation Phasing for why (keeps the confirm UX on our own branded page instead of a generic Brevo-hosted one). Inline confirmation message (same whether the honeypot caught a bot or not, since a bot doesn't need to be told it was caught) asks the user to check their inbox.
3. Clicking the confirm link hits `/api/newsletter/confirm?token=…`, which looks up the row by token, sets `confirmed_at = now()`, clears the token (single use) — **then actually subscribes the address to Brevo** (`POST /contacts`, see API Dependencies → Brevo) and marks `synced = true` on success — before redirecting to a dedicated `/newsletter/confirmed` page (a standalone page, not an inline homepage banner — see Route Structure). No token match → redirect to `/newsletter/invalid` instead, generic "isn't valid or has already been used" message, no enumeration signal.
...
| ~~`newsletter_sync_status`~~ | **Superseded, never built.** The original plan was to rename `mailchimp_sync_status` (below) to this and use it to track the approval-time Brevo subscribe outcome, with an admin-UI retry action. What actually shipped instead: `recordVerifiedNewsletterOptIn` (`src/lib/newsletter-signup.ts`) subscribes to Brevo directly and tracks success on `newsletter_signups.synced` (a different table), catching and logging failures server-side rather than surfacing a retry UI (see Implementation Phasing). `applications.mailchimp_sync_status` remains in the schema, unrenamed, and genuinely unused by any code path — dead column, not just misnamed. |
...
**Note**: this migration (and the schema/code today) still name the column and enum `mailchimp_sync_status` — that predates the Brevo decision (see Mailout Provider Comparison). The originally-planned rename to `newsletter_sync_status` **won't happen** — the real newsletter-mailout work (see Implementation Phasing) ended up tracking subscribe success on `newsletter_signups.synced` instead of an applications-table field, so this column is simply dead. Worth a future cleanup migration to drop it, not urgent.
...
Used for approval and rejection notifications to applicants, plus the newsletter confirm-link/courtesy emails (see Implementation Phasing). One-off triggered emails, distinct from Brevo's list/bulk-send side (see Mailout Provider Comparison) — but now the **same provider** for both, not a separate one.
...
- **Applied in code** — `src/lib/email.ts` calls Brevo's `POST /v3/smtp/email` directly (`fetch`, no SDK dependency, matching the pattern already used for Turnstile verification). `resend` package removed. Domain verified in Brevo (DNS records added via Netlify's DNS API — nameservers already point there, no registrar access needed, see Database section for the same finding re: the DB work); sending confirmed working end-to-end.
- **Styled** — every email wrapped in a shared branded shell (`wrapEmailHtml` in `email.ts`): cream background, white card, serif type matching the site, footer with a PNG export of the logo (not the SVG — Outlook desktop doesn't render SVG in HTML email) and a caption. All inline CSS; most clients strip `<style>` blocks and Outlook desktop doesn't reliably inherit from parent elements anyway. A protected `/admin/email-preview` page renders all four templates for a real look at them — the footer image only resolves there when `NEXT_PUBLIC_SITE_URL` points somewhere reachable, which a live inbox test against local dev can't satisfy (Gmail's servers can't fetch `localhost`), but a same-origin admin-page view can.
- **Personalized where a name exists** — approval/rejection emails greet the applicant by first name (`Hi {firstName},`, parsed from the OAuth/self-reported `name` field) when one's on file. Newsletter emails don't — standalone signups are email-only, no name is ever captured for them, so there's nothing to greet with.
- **Real sends restricted outside production**: Brevo has no sandbox key that fakes delivery without actually sending — any working `BREVO_API_KEY` sends real mail through the real account regardless of which environment triggered it (unlike `BREVO_LIST_ID`, see Brevo below, which *is* split per-context). So `send()` in `email.ts` checks Netlify's own `CONTEXT` env var (unset locally; `"production"` / `"deploy-preview"` / `"branch-deploy"` / `"dev"` on Netlify): outside `"production"`, a send only actually goes out if the recipient is in `ADMIN_EMAILS` — otherwise it's silently skipped (logged, not thrown; the calling flow completes normally either way, so testing on a preview with a non-admin address doesn't error, it just doesn't deliver). Sends that do go out outside production get `[TESTING] ` prepended to the subject, so a real notification can never be mistaken for a test one while scanning an inbox.
...
- **State Model**: `mailchimp_sync_status` → `newsletter_sync_status` rename was planned here but never happened — superseded by tracking on `newsletter_signups.synced` instead (see note under the `0001` migration, and Implementation Phasing)
...
- Env vars: `BREVO_API_KEY`, `BREVO_LIST_ID` (numeric, from Contacts → Lists in the Brevo dashboard) — **`BREVO_LIST_ID` is split per Netlify context**: `production` uses the real list, every other context (`deploy-preview`, `branch-deploy`, `dev`, `dev-server`) points at a separate staging list created purely so preview/branch testing doesn't write real signups into the production list. Two different Brevo lists, same account/API key.
- **Both mailout call sites go through the same single endpoint** — `POST /contacts` (`src/lib/brevo-contacts.ts`, `subscribeToBrevoList`), `updateEnabled: true` so re-subscribing an existing contact updates rather than errors. **Not** Brevo's `doubleOptinConfirmation` endpoint — see Implementation Phasing for why our own confirm mechanism is used instead of Brevo's native DOI:
  - Newsletter-only path: called from `/api/newsletter/confirm` once *our own* token-based confirm click succeeds — not on the original form submit
  - Work-with-us path: called from `recordVerifiedNewsletterOptIn` immediately on approval (no double opt-in needed — LinkedIn/Google OAuth already proved control of the account/email). Fired only if `newsletter_opt_in` was checked.
  - Both live, not stubbed. On failure: caught, logged server-side, no admin-facing retry surface for this one (see Implementation Phasing for why that's an acceptable, deliberate gap here but not for approve/reject notifications)
- Contact attributes can store LinkedIn URL for reference (work-with-us path only — newsletter-only signups have no LinkedIn data) — **not yet implemented**, `subscribeToBrevoList` only sends `email` today
...
**Update, post-spec: the Phase 1/Phase 2 split below is resolved.** It existed because the mailout provider was undecided; once Brevo was picked (see Mailout Provider Comparison) and real working credentials were in place (API key, verified sending domain), there was no remaining reason to stub the mailing-list subscribe calls or defer them to a later backfill — so that work was pulled forward and done directly instead. The two "stubbed" call sites described below as Phase 2 work are both live.

One deliberate choice made along the way: the standalone newsletter signup keeps **our own** double opt-in (branded confirm email + dedicated `/newsletter/confirmed` page, see Route Structure) rather than switching to Brevo's native `doubleOptinConfirmation` endpoint. Handing the confirm-click UX to Brevo would mean the applicant lands on a generic Brevo-hosted confirmation instead of the site's own branded one — a real regression for no benefit, since our mechanism already does the job. Brevo's Contacts API (`POST /contacts`) is only ever called *after* our own confirmation succeeds (or immediately for the OAuth-already-verified approval-time opt-in) — see below.
...
- Transactional emails (approval/rejection, newsletter confirm/courtesy) — Brevo transactional API, styled (see Transactional Email above), with an admin-only `/admin/email-preview` page to see all four rendered
- Full DB schema, abuse protection (honeypot, timing check, rate limiting, Cloudflare Turnstile)
- **Newsletter mailout, both call sites, real and live:**
  1. **`/api/newsletter/confirm`** (standalone signup, on confirm-link click) — after marking the `newsletter_signups` row confirmed, calls Brevo's `POST /contacts` (`src/lib/brevo-contacts.ts`) to actually subscribe the address, then sets `synced = true` on success.
  2. **Approval-time subscribe** (`newsletter_opt_in` on an application, `recordVerifiedNewsletterOptIn` in `src/lib/newsletter-signup.ts`) — subscribes to Brevo immediately (already OAuth-verified, no confirm click needed), same `synced` flag on success.
  - Both failure paths: caught and logged server-side (`console.error`), not surfaced to the admin UI or the applicant/subscriber — unlike the approve/reject notification-retry flow (see Admin UI), there's no dedicated retry surface for this one. `synced = false` is the marker a future manual sweep could act on if that's ever needed; not built preemptively.
  - `newsletter_signups` (migration `0004`) is no longer a temporary holding table awaiting a bulk-import script — it's kept on as a permanent local audit trail / sync-status record alongside the real Brevo subscription, not instead of it. Nothing to remove, no bulk-import job to write.
  - `BREVO_LIST_ID` — numeric Brevo list ID (Contacts → Lists in the Brevo dashboard) that confirmed subscribers land in. New env var.
